...
interface Bakery {
    ...
    salesOfLastWeek: Ingredients[],
    wholesalePrices: WholesaleItems[]
}
...
interface WholesaleItems extends Ingredients{
    price:number;
...
interface ListForClerk extends Ingredients{
    totalPrice:number
}
 ...
 */
const TaskSumTotalSales = (item: string): number => {
    const bakeryNotMutal : Bakery = JSON.parse(item)
    let count = 0
    const calculate : number[]= bakeryNotMutal.recipes.map(elem => {
 ...
 *             "name": "
 */
const Intolerance = (item: string): Alergies => {
    const bakeryNotMutal : Bakery = JSON.parse(item)
 ...
 */
const TaskSumTotalProfit = (item: string): Number => {
    const bakeryNotMutal : Bakery = JSON.parse(item)
    let invesment = 0
    let ingredientMultiple: Ingredients[] = []

    const calcIngredientUsage: void[] = bakeryNotMutal.salesOfLastWeek.map(element => {
 ...
 *         "name": "Muffin",
 */
const TaskCalcTotalBakeCakeableAmount = (item: string): Ingredients[] => {
    const bakeryNotMutal : Bakery = JSON.parse(item)
    const calc = bakeryNotMutal.recipes.map((element) => {
        let ingredients: Ingredients[] = element.ingredients.map((elem) => {
            let search: Ingredients[] = bakeryNotMutal.inventory.filter((elems) => elems.name == elem.name);
            let amount: number = Math.floor(
                Number(String(search[0].amount).split(" ")[0]) / Number(String(elem.amount).split(" ")[0])
            );
 ...
 */
const TaskCalcOrderForWedding = (item: string, wedding: Wedding): number => {
    const bakeryNotMutal : Bakery = JSON.parse(item)
    let moneyWasted:number = 0

    const ingredientIncrease : void[] = wedding.order.map(elem => {

        let cake : Bakery['recipes'] = bakeryNotMutal.recipes.filter(elems => elem.name == elems.name);

        let cakePrice : Bakery['wholesalePrices'] = cake[0].ingredients.map(element => {
            let ingredientPrice:Bakery['wholesalePrices'] = bakeryNotMutal.wholesalePrices.filter(el => element.name == el.name)
            return {
 ...
 */
const TaskCalcFuture = (item:string):ListForClerk[] => {
    const bakeryNotMutal :Bakery = JSON.parse(item)
    let increaseIng:any[] = []
    const cakes  : Ingredients[]= bakeryNotMutal.salesOfLastWeek.filter(element =>
    ...
    const ingredientsOnlyAndSum : Ingredients[] = increaseIngFilter.map((e) => e.ingredients).flatMap(e =>e).reduce((curr,next)=>{
    ...
    const netxTwoWeekIngredinent = () => {
        for(let item of Object.keys(ingredientsOnlyAndSum)){
            let whsale :WholesaleItems[]  = ingredientsOnlyAndSum[item].map( (e :any) => bakeryNotMutal.wholesalePrices.find(el=>e.name == el.name))
            let inventory =ingredientsOnlyAndSum[item].map((e : any) =>bakeryNotMutal.inventory.find(el=>e.name == el.name))
...
function answerTojson(data : any ,filename : string){
